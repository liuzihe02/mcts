...
from example_mcts import MCTS, Node
...
class TicTacToeBoard(_TTTB, Node):
    ...
    def make_move(board, index):
        # the turn will be a boolean here
        tup = board.tup[:index] + (board.turn,) + board.tup[index + 1 :]
        # change the turn
        turn = not board.turn

        # this is either a Boolean of None
        winner = _find_winner(tup)
        # either winner has a boolean or all values are filled up
...
def play_game():
    ...
    while True:
        ...
        # index here converts from cartesian to 0-8 scale
        index = 3 * (row - 1) + (col - 1)
